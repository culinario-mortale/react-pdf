...
import { Document, Page, View } from "@culinario-mortale/react-pdf-renderer";
...
describe("transform", () => {
  test("should scale with two args", async () => {
    const image = await renderToImage(<TestDocument transform="scale(0.5, 0.5)" />);
  ...
  test("should translate to zero px", async () => {
    const image = await renderToImage(<TestDocument width={5} height={5} transform="translateX(0px)" />);
  ...
  test("should rotate", async () => {
    const image = await renderToImage(<TestDocument width={15} height={15} transform="rotate(45deg)" />);
